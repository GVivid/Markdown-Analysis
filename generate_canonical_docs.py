...
def synthesize_canonical_content(
    sources: list[dict], num_sentences: int
) -> tuple[str, str]:
    ...
    if len(sentence_pool) <= num_sentences:
        ...
        try:
            ...
            similarities = cosine_similarity(
                sentence_vectors, cluster_centroid
            ).flatten()
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=7,
        help="Number of semantic sentences to include in generated file.",
    )
    ...
    if not db_path.exists():
        print(
            f"Error: Target database '{args.db}' does not exist. Run the deduplication script first."
        )
        return
    ...
    for cluster_id, sources in clusters.items():
        print(
            f" -> Processing Generation Phase for Cluster #{cluster_id} ({len(sources)} source assets)..."
        )

        synthetic_markdown, title_slug = synthesize_canonical_content(
            sources, args.length
        )

        file_name = (
            f"canonical_cluster_{cluster_id}_{title_slug}.md"
            if title_slug
            else f"canonical_cluster_{cluster_id}.md"
        )
